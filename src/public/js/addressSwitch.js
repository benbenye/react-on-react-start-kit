(function (window) {
    ...
    Address.prototype.bindProvinceListClick = function () {
        ...
        that.provinceListHTML.find('li').each(function () {
            ...
            li.unbind("click").bind('click', function () {
                ...
                if (that.checkIsParentId(parentId) && !that.isProductPage) {
                    that.addresObj.isParent = true;
    ...
    Address.prototype.bindDistrictListClick = function (districtListHTML, provinceId, cityId) {
        ...
        that.districtListHTML.find('li').each(function () {
            ...
            liDistrict.unbind("click").bind('click', function () {
                ...
                that.tabDistrict.text(districtName);

                that.addresObj.three = {
                    tid: districtId,
                    name: districtName
                }
                // 
    ...
    Address.prototype.setAddressInfoFromCookie = function () {
        ...
        if (!isNaN(provinceId) && provinceId > 0) {
            ...
            if (currentLiParent.length > 0) {//北京，上海等
                ...
                that.tabProvince.text(currentLiParent.text());
                ...
                that.tabProvince.text(currentLiProvince.text());
            }
    ...
    Address.prototype.setCityInfoFromCookie = function (provinceId, cityId) {
        ...
        //设置
        var cityListHTML = $("#cityList");
        var tabCity = $("#tabCity");
        var currentLi = cityListHTML.find('li[data-city="' + cityId + '"]');
        currentLi.addClass('on');
        currentLi.siblings().removeClass('on');
        that.tabCity.text(currentLi.text());
    };
    ...
    Address.prototype.setDistrictInfoFromCookie = function (provinceId,cityId, districtId) {
        ...
        currentLi.addClass('on');
        currentLi.siblings().removeClass('on');
        that.tabDistrict.text(currentLi.text());
    };
    ...
    Address.prototype.writeHotCityToCookie = function (siteId, cityId, cityName, provinceId, districtId) {
        ...
        $.cookie("cb_site_name", (that.addresObj.isParent ? that.addresObj.one.name : that.addresObj.two.name), {expires: date_history, path: "/", domain: cookieDomain});
        ...
        if (typeof districtId != 'undefined') {
            $.cookie("cb_address_district", districtId, {expires: date_history, path: "/", domain: cookieDomain});
            
            var streetList =  that.getAddrListById(districtId);
            $.cookie("cb_address_street", streetList[0]['tid'], {expires: date_history, path: "/", domain: cookieDomain});
            $.cookie("cb_address_text", (that.addresObj.one.name+that.addresObj.two.name+that.addresObj.three.name), {expires: -1, path: "/", domain: cookieDomain});
        }
        else {
